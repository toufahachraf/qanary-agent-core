...
from core.state import WorkflowStage, PatientContext, MRIMetadata
...
async def process_chat(request: ChatRequest):
    ...
    try:
        ...
        if request.patient_context:
            # Convert JSON dict to strict Pydantic object for the agent's memory
            input_state["patient"] = PatientContext(**request.patient_context.model_dump())
            
        if request.mri_gcs_uri:
            input_state["mri_data"] = MRIMetadata(
